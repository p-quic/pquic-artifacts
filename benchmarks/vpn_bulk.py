    ...
    for size in file_sizes:
        ...
        def run():
            ...
            err = comp.run_cmd_on_client(nodes, 'curl {}:8080/random_{} -s --connect-timeout 5 --output /dev/null -w "%{{time_total}}" {} > /tmp/curl.log'.format(
                '128.0.0.1' if "vpn" in test_name else server_ip, size, additional_curl_params)
            )
...
if __name__ == "__main__":
    test_nets_opts = {
        ...
        'sp_vpn': {'multipath': False, 'log_server': "/dev/null"},
        'mp_vpn': {'multipath': True, 'log_server': "/dev/null"},
    }
    ranges = {
        "bw_a": {"range": [5, 50], "type": float, "count": 1},  # Mbps
        # "loss_a": {"range": [0.1, 2], "type": float, "count": 1},  # %, TODO: Characterise typical losses with LTE
        "delay_ms_a": {"range": [2.5, 25], "type": float, "count": 1},  # ms
        "bw_b": {"range": [5, 50], "type": float, "count": 1},  # Mbps
        # "loss_b": {"range": [0.01, 1], "type": float, "count": 1},  # %
        "delay_ms_b": {"range": [2.5, 25], "type": float, "count": 1},  # ms
    }
    mp_ranges = {
        "bw_a": {"range": [1, 15], "type": float, "count": 1},  # Mbps
        # "loss_a": {"range": [0.1, 2], "type": float, "count": 1},  # %, TODO: Characterise typical losses with LTE
        "delay_ms_a": {"range": [2.5, 25], "type": float, "count": 1},  # ms
        "bw_b": {"range": [1, 15], "type": float, "count": 1},  # Mbps
        # "loss_b": {"range": [0.01, 1], "type": float, "count": 1},  # %
        "delay_ms_b": {"range": [2.5, 25], "type": float, "count": 1},  # ms
    }
    ...
    run_experimental_design(test_nets_opts, ranges, run_vpn,
                            db_filename='results_vpn_bulk.db',
